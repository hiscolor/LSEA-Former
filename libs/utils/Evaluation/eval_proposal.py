...
class ANETproposal(object):
    ...
    def _import_ground_truth(self, ground_truth_filename):
        ...
        for vid_key, v in data.items():

            if 'file' in v:
                videoid = os.path.basename(v['file']).replace('.mp4','').replace('.wav','')
            else:
                videoid = vid_key


            vid_split = v.get('split', v.get('subset', '')).lower()
            if self.subset != vid_split:
                continue
            if videoid in self.blocked_videos:
                continue


            annotations = v.get('fake_periods', v.get('annotations', []))
            if not annotations:
                continue
...
def average_recall_vs_avg_nr_proposals(ground_truth, proposals,
    ...
    videos_with_proposals = set(proposals['video-id'].unique())
    ...
    for videoid in video_lst:
        ...
        if videoid not in videos_with_proposals:
